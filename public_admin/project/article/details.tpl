  ...
  <div id="content">
    ...
    <div id="sidetabs">
        <ul> 
			...
			<li><a href="{if isset($articleData)}/project/article/comments.php?code={$articleData.article_code}{/if}" title="Comments">Comments</a></li>
        </ul>
    </div><!--tabs-->
	<div class="detail_box">
      <form id="detailsForm" name="detailsForm" action="/project/article/details.php{if isset($articleData)}?code={$articleData.article_code}{/if}" method="post" >
        <table width="700" border="0" align="center" cellpadding="0" cellspacing="0" class="form">
		  ...
		  <tr>
			<td colspan="3">				
				...
				<input type="text" name="project_name" id="project_name" value="{$articleData.project_name}" size="120" />
				<input type="hidden" name="project_code" id="project_code" value="{$articleData.project_code}" />
				{else}
				<span class="success">{$articleData.project_name}</span>
				<input type="hidden" name="project_code" id="project_code" value="{$articleData.project_code}" />				
				{/if}
				{if isset($errorArray.project_code)}<br /><span class="error">{$errorArray.project_code}</span>{else}<br />Project owner of the article{/if}
			</td>			  
		  </tr>
          <tr>	
