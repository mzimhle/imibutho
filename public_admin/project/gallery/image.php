	...
	if(trim($errorArray) == '' && $formValid == true) {
		...
		if(isset($_FILES['imagefile']) && count($_FILES['imagefile']['name']) > 0) {
			for($i = 0; $i < count($_FILES['imagefile']['name']); $i++) {				
				...
				$directory	= realpath(__DIR__.'/../../../public_html/').'/media/project/'.$galleryData['project_code'].'/gallery/'.$galleryData['gallery_code'].'/'.$data['galleryimage_code'];
				...
				$data['galleryimage_path']			= '/media/project/'.$galleryData['project_code'].'/gallery/'.$galleryData['gallery_code'].'/'.$data['galleryimage_code'];
		...
		header('Location: /project/gallery/image.php?code='.$galleryData['gallery_code']);
		exit;
	}
...
$smarty->display('project/gallery/image.tpl');
