  ...
  <div id="content">
    ...
    <div id="sidetabs">
        <ul> 
			...
			<li><a href="{if isset($eventData)}/project/event/comments.php?code={$eventData.event_code}{/if}" title="Comments">Comments</a></li>
        </ul>
    </div><!--tabs-->
	<div class="detail_box">
      <form id="detailsForm" name="detailsForm" action="/project/event/details.php{if isset($eventData)}?code={$eventData.event_code}{/if}" method="post" >
        <table width="700" border="0" align="center" cellpadding="0" cellspacing="0" class="form">
		  ...
		  <tr>
			<td colspan="3">				
				...
				{if !isset($eventData)} 
				<input type="text" name="project_name" id="project_name" value="{$eventData.project_name}" size="120" />
				<input type="hidden" name="project_code" id="project_code" value="{$eventData.project_code}" />
				{else}
				<span class="success">{$eventData.project_name}</span>
				<input type="hidden" name="project_code" id="project_code" value="{$eventData.project_code}" />				
				{/if}
				{if isset($errorArray.project_code)}<br /><span class="error">{$errorArray.project_code}</span>{else}<br />Project owner of the event{/if}
			</td>			  
		  </tr>		  
          <tr>	
