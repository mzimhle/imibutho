...
class class_account extends Zend_Db_Table_Abstract
{
   //declare table variables
	...
	public function insert(array $data) {
		...
		$code = parent::insert($data);	
		
		if($code) {
			
			$accountData = $this->getByCode($code);
			
			if($accountData) {
				
				$this->_comm->sendAccount($accountData, 'REGISTER_EMAIL', 'Imibutho Forum Registration Activation', $_SERVER['DOCUMENT_ROOT'].'/templates/account_register.html');
	...
	public function getByHash($code, $activate) {
		$select = $this->_db->select()	
					...
					->where('account.account_hashcode = ?', $code)
					->where('account_deleted = 0')
					->where('account.account_active = ?', $activate)
					->limit(1);
